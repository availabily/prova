...
function SampleCertCard({ id, verdict, label, confidence, failureType, snippet }: {
  ...
  return (
    <Wrapper
      {...wrapperProps as any}
      className={`block border border-border transition-all duration-200 p-6 space-y-3 bg-surface/30 ${
        isReal ? 'hover:border-muted hover:bg-surface/50 group cursor-pointer' : 'opacity-80'
      }`}
    >
      ...
      <div className="mono text-xs text-muted group-hover:text-dim transition-colors duration-200">
        {isReal ? id : `${id} (demo)`}
      </div>
    </Wrapper>
  )
